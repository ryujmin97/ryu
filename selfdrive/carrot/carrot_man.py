...
class CarrotMan:
  def __init__(self):
    ...
    #
    # 대응: 지평선을 4.5s -> 6.5s로 2초 확대(사용자 요청)해 더 먼 지점까지
    # 필요속도 계산에 포함시켜, 급하게 조여드는 커브의 정점을 더 일찍
    # "본" 상태로 감속을 시작하게 한다. 모델 예측 궤적(ModelConstants.
    # T_IDXS) 자체가 최대 10.0s까지 있으므로 6.5s는 모델 데이터 범위
    # 안에서 안전하게 늘릴 수 있는 값이다.
    # 주의: 이번 조정만으로는 8.6초짜리 극단적으로 긴 조임 구간 전체를
    # 커버하지 못할 수 있음(6.5s < 8.6s) -- 표본 1건 기반 조정이라 다음
    # 세션에서 이 값 적용 후 실주행으로 재검증 필요 (devnotes
    # PARAMS_REGISTRY.md 참고).
    self.vturn_lookahead_horizon_s = 6.5  # 진입 조기감속용 예측 구간(초). T_IDXS가 비선형이라 '초' 기준으로 계산한다.
